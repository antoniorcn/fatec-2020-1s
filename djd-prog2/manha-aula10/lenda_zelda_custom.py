...
img_vaso_orig = tiles.subsurface((48, 48), (16, 16))
img_vaso = pygame.transform.scale(img_vaso_orig, (256, 256))
img_grama = load_image(tiles, 16, 128)
img_parede = load_image(tiles, 48, 0)


def teste_colisao_mapa(personagem, map, mapa_caracteres):
        colisoes = []
        for id_linha, linha in enumerate(map):
                for id_coluna, caracter in enumerate(linha):
                        if caracter in mapa_caracteres.keys():
                                size = mapa_caracteres[caracter]
...
class Camera:
        def __init__(self, position, tamanho):
                ...
                # self.draw_area = self.draw_area.convert_alpha()
                self.draw_area.set_colorkey(PRETO)
                #self.draw_area.set_alpha(0)
...
class Personagem(pygame.sprite.Sprite):
        ...
        def update(self):
                ...
                colisoes_movimento = teste_colisao_mapa(self, mapa, {"p": (BLK_WIDTH, BLK_HEIGHT)})
                if len(colisoes_movimento) == 0:
                        self.rect.move_ip(self.vel_x, self.vel_y)

                colisoes = teste_colisao_mapa(self, mapa_objs, {"v": (256, 256)})
...
while True:
        cam.start_drawing()
        cam.draw_tiles(mapa, {"p": img_parede})
        cam.draw_tiles(mapa_objs, {"v": img_vaso})

        parallax(tela, (0, 0), space, heroi.rect.centerx)
